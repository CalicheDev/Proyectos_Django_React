...
export default function DataForm() {
  ...
  //let [authToken, setAuthTokens] = useState(()=> Cookies.get('auth_token') ? JSON.parse(Cookies.get('auth_token')) : null)
  ...
  useEffect(() => {
    ...
    if (authToken) {        
      ...
      if (decodedToken) {
        const { username, password } = decodedToken; // Obtener los datos de usuario del token decodificado
        setUser({ displayName: username, password }); // Actualizar el estado con los datos de usuario
      }
    }
  }, []);
